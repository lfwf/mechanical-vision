...
function FrontPanelFrame() {
  return (
    <group>
      <RoundedBox args={[4.46, 0.68, 0.12]} radius={0.05} smoothness={4} position={[0, 2.03, 0]} castShadow>
        <meshStandardMaterial color={sheetColor} metalness={0.16} roughness={0.38} />
      </RoundedBox>
      <RoundedBox args={[4.46, 1.02, 0.12]} radius={0.05} smoothness={4} position={[0, -1.94, 0]} castShadow>
        <meshStandardMaterial color={sheetColor} metalness={0.16} roughness={0.38} />
      </RoundedBox>
      <RoundedBox args={[0.48, 3.38, 0.12]} radius={0.045} smoothness={4} position={[-1.99, 0.04, 0]} castShadow>
        <meshStandardMaterial color={sheetColor} metalness={0.16} roughness={0.38} />
      </RoundedBox>
      <RoundedBox args={[0.48, 3.38, 0.12]} radius={0.045} smoothness={4} position={[1.99, 0.04, 0]} castShadow>
        <meshStandardMaterial color={sheetColor} metalness={0.16} roughness={0.38} />
      </RoundedBox>
      <mesh position={[0, 0.24, -0.015]}>
        <torusGeometry args={[1.5, 0.11, 18, 72]} />
        <meshStandardMaterial color="#d7dbd7" metalness={0.18} roughness={0.4} />
      </mesh>
      <RoundedBox args={[0.72, 0.4, 0.08]} radius={0.04} smoothness={4} position={[-1.62, -1.7, 0.08]}>
        <meshStandardMaterial color="#d3d7d3" roughness={0.44} />
      </RoundedBox>
    </group>
  );
}
...
function CabinetShell() {
  return (
    <group>
      ...
      {[-2.23, 2.23].map((x) => (
        <group key={x} position={[x, 0, 0]}>
          ...
          <mesh position={[x < 0 ? 0.07 : -0.07, 0, 0]}>
            <boxGeometry args={[0.025, 4.55, 3.7]} />
            <meshStandardMaterial color="#d5d9d5" metalness={0.12} roughness={0.5} />
          </mesh>
        </group>
      ))}
      ...
      {[-1.55, 1.55].map((x) => (
        <mesh key={x} position={[x, -2.42, 1.58]}>
          <boxGeometry args={[0.42, 0.18, 0.5]} />
          <meshStandardMaterial color="#4b5a5d" metalness={0.45} roughness={0.32} />
        </mesh>
      ))}
      {[-1.55, 1.55].map((x) => (
        <mesh key={`rear-${x}`} position={[x, -2.42, -1.58]}>
          <boxGeometry args={[0.42, 0.18, 0.5]} />
          <meshStandardMaterial color="#4b5a5d" metalness={0.45} roughness={0.32} />
        </mesh>
      ))}
    ...
    <group>
      ...
      {assemblyEnabled && (
        <>
          <ExplodablePart id="wm-top-cover" home={[0, 2.68, 0]} exploded={[0, 4.2, -0.55]} assemblyEnabled selectionRadius={1.95}>
            <RoundedBox args={[4.58, 0.16, 4.16]} radius={0.08} smoothness={5} castShadow>
              <meshStandardMaterial color="#f0f1ed" metalness={0.12} roughness={0.34} />
            </RoundedBox>
          </ExplodablePart>
          <ExplodablePart id="wm-front-panel" home={[0, -0.05, 2.13]} exploded={[0, -0.05, 4.85]} assemblyEnabled selectionRadius={2.1}>
            <FrontPanelFrame />
          </ExplodablePart>
          <ExplodablePart id="wm-rear-cover" home={[0, 0, -2.11]} exploded={[0, 0, -4.85]} assemblyEnabled selectionRadius={2.1}>
            <RoundedBox args={[4.4, 4.82, 0.1]} radius={0.05} smoothness={4} castShadow>
              <meshStandardMaterial color="#d9ddd9" metalness={0.16} roughness={0.4} />
            </RoundedBox>
            <mesh position={[0, 0, -0.06]}>
              <torusGeometry args={[1.55, 0.05, 12, 64]} />
              <meshStandardMaterial color="#b6bfbc" metalness={0.28} roughness={0.35} />
            </mesh>
          </ExplodablePart>
        </>
      )}
      ...
      {showUpperControls && (
        <>
          <ExplodablePart id="wm-control-panel" home={[0, 2.04, 2.1]} exploded={[0, 3.65, 3.5]} assemblyEnabled={assemblyEnabled} selectionRadius={1.75}>
            <RoundedBox args={[4.38, 0.68, 0.28]} radius={0.08} smoothness={5} castShadow>
              <meshStandardMaterial color="#e8eae6" metalness={0.12} roughness={0.34} />
            </RoundedBox>
            <mesh position={[0.25, 0, 0.17]} rotation={[Math.PI / 2, 0, 0]}>
              <cylinderGeometry args={[0.3, 0.3, 0.12, 40]} />
              <meshStandardMaterial color="#52696e" metalness={0.45} roughness={0.28} />
            </mesh>
            <RoundedBox args={[1.08, 0.28, 0.05]} radius={0.025} smoothness={3} position={[1.38, 0, 0.17]}>
          ...
          <ExplodablePart id="wm-detergent-drawer" home={[-1.42, 2.04, 2.28]} exploded={[-1.42, 2.04, 4.35]} assemblyEnabled={assemblyEnabled} selectionRadius={0.55}>
            <RoundedBox args={[1.12, 0.44, 0.66]} radius={0.05} smoothness={4} castShadow>
              <meshStandardMaterial color="#edf0eb" roughness={0.38} />
            </RoundedBox>
          </ExplodablePart>
        </>
      )}
      ...
      {(assemblyEnabled || mode === WashingMachineMode.DriveCutaway) && (
        <ExplodablePart id="wm-main-pcb" home={[1.35, 1.9, -1.55]} exploded={[3.4, 3.0, -3.0]} assemblyEnabled={assemblyEnabled} selectionRadius={0.7}>
          <RoundedBox args={[1.25, 0.82, 0.52]} radius={0.05} smoothness={4} castShadow>
            <meshStandardMaterial color="#4a6267" metalness={0.28} roughness={0.35} />
          </RoundedBox>
          <mesh position={[0, 0, 0.28]}>
            <boxGeometry args={[0.98, 0.58, 0.035]} />
            <meshStandardMaterial color="#6ba079" roughness={0.45} />
          </mesh>
        </ExplodablePart>
      )}
    </group>
